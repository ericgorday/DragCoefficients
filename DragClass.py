...
class Drag_C_Calc(object):
	...
	def find_bouyancy(self,choice):  # Initial function used to upward force of buoyancy (used in determining drag in Z axis)
	...
	def apply_force(self,choice):
	    ...
	    while not (rospy.is_shutdown()):
	        ...
	        if Compare_Velocities < self.delta:
	            ...
	            rospy.loginfo('Linear Drag Values-- x: {},  y: {},  z: {}'.format(self.linear_drag_x, self.linear_drag_y, self.linear_drag_z))
	            rospy.loginfo('Rotational Drag Values-- yaw: {},  pitch: {},  roll: {}'.format(self.yaw_drag, self.pitch_drag, self.roll_drag))
	...
	def fileOutput(self):
		drag = {'x': self.linear_drag_x, 'y': self.linear_drag_y, 'z': self.linear_drag_z, 'yaw': self.yaw_drag, 'pitch': self.pitch_drag, 'roll': self.roll_drag}
